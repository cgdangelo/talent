...
describe('Parser', () => {
  test('skip', () => {
    ...
    expect(pipe(stream(new Array(20), 10), P.skip(10))).toStrictEqual(
      PR.success(undefined, stream(new Array(20), 10), stream(new Array(20), 20))
    );
  ...
  test('seek', () => {
    ...
    expect(pipe(stream(new Array(100), 100), P.seek(1))).toStrictEqual(
      PR.success(undefined, stream(new Array(100), 100), stream(new Array(100), 1))
    );
  ...
  test('manyN1', () => {
    ...
    expect(pipe(stream([]), P.manyN1(P.item(), 0))).toStrictEqual(PR.error(stream([])));

    expect(pipe(stream([]), P.manyN1(P.item(), 6))).toStrictEqual(PR.error(stream([])));
  });
  ...
  test('manyN', () => {
    ...
    expect(pipe(stream([]), P.manyN(P.item(), 6))).toStrictEqual(PR.success([], stream([]), stream([])));
  });
  ...
  test('struct', () => {
    expect(pipe(stream([]), P.struct({ a: P.of('a'), b: P.of('b') }))).toStrictEqual(
      PR.success({ a: 'a', b: 'b' }, stream([]), stream([]))
    );

    expect(pipe(stream([]), P.struct({ a: P.of('a'), b: P.fail() }))).toStrictEqual(PR.error(stream([])));
  });
  ...
  test('tuple', () => {
    ...
    expect(pipe(stream([]), P.tuple(P.of('a'), P.fail()))).toStrictEqual(PR.error(stream([])));
  });
});
