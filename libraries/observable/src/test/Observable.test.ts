...
const test$: (name: string, f: (helpers: RunHelpers) => void) => void = (name, f) =>
  test(name, () => testScheduler.run(f));

describe('Observable', () => {
  describe('constructors', () => {
    test$('from', ({ expectObservable }) => expectObservable(_.from([1, 2])).toBe('(ab|)', { a: 1, b: 2 }));

    test$('of', ({ expectObservable }) => expectObservable(_.of(1)).toBe('(a|)', { a: 1 }));

    test$('zero', ({ expectObservable }) => expectObservable(_.zero()).toBe('(|)'));
  });
